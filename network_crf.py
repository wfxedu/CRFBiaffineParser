...
def TensorToFile(outfd,tx,idx):
  shp = tx.shape
  is_res = (len(shp)>3)
  for bh_id in range(0,shp[0]):
   outfd.write('##%d\n' % idx)
   for w1 in range(0,shp[1]):
      for w2 in range(0,shp[2]):
         if is_res:
            for ir in range(0,shp[3]):
               outfd.write('%f\t' % tx[bh_id][w1][w2][ir])
            outfd.write('\n')
         else:
            outfd.write('%f\n'% tx[bh_id][w1][w2])
   outfd.write('\n')
...
#    return (3, 4, 5)
def SenToFile(outfd,sen_input,sen_target,idx,vocabs,sents):
   shp = sen_input.shape
   words, tags, rels = vocabs
   for bh_id in range(0,shp[0]):
      #outfd.write('##%d\n'% idx)
      for wd in range(1,shp[1]):
         itm1 = sen_input[bh_id][wd]
         itm2 = sen_target[bh_id][wd]
         if itm1[0]==0:
            break
         word = sents[bh_id][wd-1]
         #word = words[itm1[0]]
         POS = tags[itm1[2]]
         head = int(itm2[1])
         REL = rels[itm2[2]]
         #2	,	_	,	,	_	7	punct	_	_
         outfd.write('%d\t%s\t_\t%s\t%s\t_\t%d\t%s\t_\t_\t\n'% (wd,word,POS,POS,head,REL) )
      outfd.write('\n')
      idx=idx+1
   return idx

def SaveRelMap(opath,rel_map):
   outfd = open(opath,'wb')
   for key in sorted(rel_map):
      v = rel_map[key]
      outfd.write('%s\t'%( key))
...
class Network(Configurable):
  ...
  def train(self, sess):
    ...
    #outfile1 = open('score_arc.test','wb')
    #outfile2 = open('score_rel.test','wb')
    #outfile3 = open('sen_conll.test','wb')
    #outfilew = open('score_w.test','wb')
    #SaveRelMap('rel_map.test',self._vocabs[2])
    #------------------------------------------------------------------
    idx_arc=0
    idx_rel=0
    idx_sen=0
    ...
    try:
      ...
      while total_train_iters < train_iters:
        for j, (feed_dict, sents) in enumerate(self.train_minibatches()):
          ...
          if False:
             print("skip training")
             #idx_arc=TensorToFile(outfile1,log_arc_probs,idx_arc)# 
             #idx_rel=TensorToFile(outfile2,log_rel_probs,idx_rel)# 
             #idx_rel1=TensorToFile(outfilew,np_w,idx_rel1)# 
             #idx_sen=SenToFile(outfile3,train_inputs,train_targets,idx_sen,self._vocabs,sents) #
             #exit()
          #------------------------------------------------------------------
          ...
          if total_train_iters == 1 or total_train_iters % validate_every == 0:
            ...
            with open(os.path.join(self.save_dir, 'sanitycheck.txt'), 'w') as f:
              for k, (feed_dict, sents) in enumerate(self.valid_minibatches()):
  ...
  def test(self, sess, validate=False):
    ...
    if validate:
      ...
      op = self.ops['test_op'][0][0]+self.ops['test_op'][0][1]
    else:
      filename = self.test_file
      minibatches = self.test_minibatches
      dataset = self._testset
      op = self.ops['test_op'][1][0]+self.ops['test_op'][1][1]
    ...
    for (feed_dict, sents) in minibatches():
      ...
      mb_pred, mb_rel,mb_arcprobs,mb_relprobs = sess.run(op, feed_dict=feed_dict)
      
      #-----------------------------
      mb_probs = [mb_arcprobs,mb_relprobs]
      print(mb_probs[0].shape,mb_probs[1].shape,type(mb_probs[1]),mb_inputs.shape,sents.shape )
      print(sents)
      idx_arc=TensorToFile(outfile1,mb_probs[0],idx_arc) 
      idx_rel=TensorToFile(outfile2,mb_probs[1],idx_rel) 
      idx_sen=SenToFile(outfile3,mb_inputs,mb_targets,idx_sen,self._vocabs,sents) 
      assert(idx_sen==idx_rel)
      assert(idx_sen==idx_arc)
      #-----------------------------
      
      tresult,batch_heads,batch_rels=self.model.validate_result(mb_inputs, mb_targets, [mb_pred, mb_rel])
      all_predictions[-1].extend(tresult)
  ...
  def _gen_ops(self):
    ...
    ops['test_op'] = [[valid_output['predictions_test'],valid_output['probabilities_test']],
...
if __name__ == '__main__':
  """"""
  sys.argv = sys.argv+'--config_file config/myconfig_PTBYM.cfg --save_dir saves/mymodel_PTBYMdel'.split(' ')

...
class BaseParser(NN):
  ...
  def validate(self, mb_inputs, mb_targets, mb_probs):
    ...
    for inputs, targets, parse_probs, rel_probs in zip(mb_inputs, mb_targets, mb_parse_probs, mb_rel_probs):
      ...
      #print(parse_probs.dtype,rel_probs.dtype,type(length))
      parse_preds, rel_preds = None,None # error crflib.decode(parse_probs, rel_probs, length)
      ...
      sents.append(sent)
      batch_heads.append(parse_preds)
      batch_rels.append(rel_preds)
    return sents,batch_heads,batch_rels
    
  def validate_result(self, mb_inputs, mb_targets, mb_prediction):
    """"""
    
    sents = []
    batch_heads=[]
    batch_rels=[]
    [mb_pred, mb_rel] = mb_prediction
    for inputs, targets, parse_preds, rel_preds in zip(mb_inputs, mb_targets, mb_pred, mb_rel):
      tokens_to_keep = np.greater(inputs[:,0], Vocab.ROOT)
      length = np.sum(tokens_to_keep)
      #parse_preds, rel_preds = self.prob_argmax(parse_probs, rel_probs, tokens_to_keep)
      #print(parse_probs.dtype,rel_probs.dtype,type(length))
      #print(type(parse_all))
      #parse_preds, rel_preds = parse_all
